...
func (c *Client) Release(ctx echo.Context, request createorder.Request) error {
	type Request struct {
		UUID     string `json:"uuid"`
		Quantity int    `json:"quantity"`
	}
	...
	_, err = c.doRequest(ctx.Request().Context(), http.MethodPost, c.baseUrl+"/release", bytes.NewBuffer(jsonStr))
	if err != nil {
		return errors.Wrap(err, "executing http request")
	}
